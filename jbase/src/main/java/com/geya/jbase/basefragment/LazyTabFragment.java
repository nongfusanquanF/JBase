 ...
 * @author LuckyJayce
 */
public class LazyTabFragment extends BaseTabFragment {
    ...
    public void setUserVisibleHint(boolean isVisibleToUser) {
        ...
        if (isVisibleToUser && !isInit && getRootView() != null) {
            //isInit init initent initent=new initent;
            isInit = true;
            onCreateViewLazy(savedInstanceState);
            onResumeLazy();
        }
        if (isInit && getRootView() != null) {
            if (isVisibleToUser) {
                isStart = true;
                onFragmentStartLazy();
            } else {
    ...
    public void setRootView(int layoutResID) {
        if (isLazyLoad && getRootView() != null && getRootView().getParent() != null) {
            layout.removeAllViews();
            View view = inflater.inflate(layoutResID, layout, false);
            layout.addView(view);
        } else {
            super.setRootView(layoutResID);
        }
    }
